...
test = false;   % false => testfila, true => hovedfila
fprintf('\nStarter innlesning...\n')       % Melding til bruker
...
elem = fscanf(filid,'%i %i %f %i',[4 nelem])';  % Informasjon om elementene

fprintf('Noder og elementer lest inn\n')        % Melding til bruker

%% Tversnitt
nTver = fscanf(filid, '%i', [1 1]);             % Antall tversnitt
profil = fscanf(filid,'%i %f %f %f %f %f %f',[7 nTver])'; % Om tversnittene 
